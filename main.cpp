...
int main() {
    ...
    while(1) {
        // status
        Brain.Screen.printAt( 10, 20, "Packets   %d", pi_comms.get_packets() );
        Brain.Screen.printAt( 10, 40, "Errors    %d", pi_comms.get_errors() );
        Brain.Screen.printAt( 10, 60, "Timeouts  %d", pi_comms.get_timeouts() );

        // once per second, update data rate
        if( ++loops == 1000/loop_time ) {
          loops = 0;
          Brain.Screen.printAt( 10, 80, "data/sec  %d             ", pi_comms.get_total() - last_data);
          Brain.Screen.printAt( 10,100, "pkts/sec  %d             ", pi_comms.get_packets() - last_packets);
          last_data = pi_comms.get_total();
          last_packets = pi_comms.get_packets();
        } 

        // get last map data
        pi_comms.get_data( &local_map );
